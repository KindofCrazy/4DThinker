...
class GenerationMixin:
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # Latent mode init — per-batch-item state to support batch_size > 1
        cur_bs = input_ids.shape[0]
        _device = input_ids.device
        in_latent_mode = torch.zeros(cur_bs, dtype=torch.bool, device=_device)
        latent_just_started = torch.zeros(cur_bs, dtype=torch.bool, device=_device)
        latent_num = torch.zeros(cur_bs, dtype=torch.long, device=_device)
        latent_hidden_states = None
        cur_latent_embeds = None  # [B, 1, H] cached hidden states for latent-mode items
        is_prefill = True
        MAX_LATENT_LEN = getattr(self.config, "latent_size", 8)
        latent_start_idx = getattr(self.config, "latent_start_id", 151666)
        latent_end_idx = getattr(self.config, "latent_end_id", 151667)
        latent_pad_idx = getattr(self.config, "latent_token_id", 151665)

        while self._has_unfinished_sequences(
            this_peer_finished, synced_gpus, device=input_ids.device, cur_len=cur_len, max_length=max_length
        ):
            any_latent = in_latent_mode.any().item()
            any_just_started = latent_just_started.any().item()

            if not any_latent:
                # Normal mode — all items use input_ids
                model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            elif any_just_started:
                # First step after latent_start — latent_start token needs normal embedding
                model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
                latent_just_started.zero_()
            else:
                # Continuing latent mode — build mixed inputs_embeds
                last_token_ids = input_ids[:, -1:]  # [B, 1]
                token_embeds = self.get_input_embeddings()(last_token_ids)  # [B, 1, H]
                # For items in latent mode, replace with cached hidden states
                mixed_embeds = torch.where(
                    in_latent_mode.view(-1, 1, 1).expand_as(token_embeds),
                    cur_latent_embeds,
                    token_embeds,
                )
                model_inputs = self.prepare_inputs_for_generation(
                    input_ids, inputs_embeds=mixed_embeds, **model_kwargs
                )
            ...
            # --- Per-item latent token detection (vectorized) ---
            is_start = (next_tokens == latent_start_idx)  # [B]
            is_end = (next_tokens == latent_end_idx)       # [B]

            # Items hitting latent_start: enter latent mode, reset counter
            newly_started = is_start & ~in_latent_mode
            if newly_started.any():
                in_latent_mode[newly_started] = True
                latent_just_started[newly_started] = True
                latent_num[newly_started] = 0
                # Reset latent_hidden_states when first item enters latent mode
                if not in_latent_mode[~newly_started].any():
                    latent_hidden_states = None
                print(f"[DEBUG-LATENT] Entered latent mode: {newly_started.sum().item()}/{cur_bs} items, cur_len={cur_len}", flush=True)

            # Items hitting latent_end: exit latent mode
            if is_end.any():
                in_latent_mode[is_end] = False
                if not in_latent_mode.any():
                    print(f"[DEBUG-LATENT] All items exited latent mode (latent_end), cur_len={cur_len}", flush=True)

            # Items in latent mode (not start/end boundary): force next_token to latent_pad
            in_latent_not_boundary = in_latent_mode & ~is_start & ~is_end
            next_tokens[in_latent_not_boundary] = latent_pad_idx

            # Count latent steps for active items (not just started)
            active_counting = in_latent_mode & ~latent_just_started
            latent_num[active_counting] += 1

            # Force-exit items exceeding MAX_LATENT_LEN
            exceeded = (latent_num > MAX_LATENT_LEN) & in_latent_mode
            if exceeded.any():
                in_latent_mode[exceeded] = False
                next_tokens[exceeded] = latent_end_idx
                print(f"[DEBUG-LATENT] Force-exited {exceeded.sum().item()} items (MAX_LATENT_LEN={MAX_LATENT_LEN}), cur_len={cur_len}", flush=True)

            # Append tokens to input_ids
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)

            # Accumulate latent_hidden_states for items in latent mode
            if in_latent_mode.any() and not latent_just_started.any():
            ...
            # Cache hidden states for next iteration's mixed-mode input
            if in_latent_mode.any() and not latent_just_started.any():
                cur_latent_embeds = outputs.hidden_states  # [B, 1, H]
