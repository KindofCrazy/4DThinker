...
class Qwen2_5_VLVisionAttention(nn.Module):
    ...
    def forward(
        ...
        # Batch attention: split by cu_seqlens, pad, batch, attend, unpad
        num_segments = len(cu_seqlens) - 1
        seg_lens = (cu_seqlens[1:] - cu_seqlens[:-1]).tolist()
        max_seqlen = max(seg_lens)
        head_dim = q.shape[-1]

        all_same_len = all(s == seg_lens[0] for s in seg_lens)

        if all_same_len:
            # Fast path: no padding needed
            q_batch = q.reshape(num_segments, max_seqlen, self.num_heads, head_dim).transpose(1, 2)
            k_batch = k.reshape(num_segments, max_seqlen, self.num_heads, head_dim).transpose(1, 2)
            v_batch = v.reshape(num_segments, max_seqlen, self.num_heads, head_dim).transpose(1, 2)

            attn_weights = torch.matmul(q_batch, k_batch.transpose(2, 3)) / math.sqrt(self.head_dim)
            attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(q.dtype)
            attn_output = torch.matmul(attn_weights, v_batch)
            attn_output = attn_output.transpose(1, 2).reshape(seq_length, self.num_heads, head_dim)
        else:
            # General path: pad segments
            q_batch = q.new_zeros(num_segments, max_seqlen, self.num_heads, head_dim)
            k_batch = k.new_zeros(num_segments, max_seqlen, self.num_heads, head_dim)
            v_batch = v.new_zeros(num_segments, max_seqlen, self.num_heads, head_dim)
            attn_bias = torch.full(
                (num_segments, 1, max_seqlen, max_seqlen),
                torch.finfo(q.dtype).min, device=q.device, dtype=q.dtype,
            )

            for i in range(num_segments):
                start = cu_seqlens[i].item()
                slen = seg_lens[i]
                q_batch[i, :slen] = q[start:start + slen]
                k_batch[i, :slen] = k[start:start + slen]
                v_batch[i, :slen] = v[start:start + slen]
                attn_bias[i, :, :slen, :slen] = 0
            ...
            parts = []
            for i in range(num_segments):
                parts.append(attn_output[i, :seg_lens[i]])
            attn_output = torch.cat(parts, dim=0)
...
class Qwen2_5_VLVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        # Check if all segments have the same length (common case: repeated identical videos)
        all_same_len = all(s == seg_lens[0] for s in seg_lens)

        if all_same_len:
            # Fast path: no padding needed, just reshape into batch
            # q, k, v: [seq_length, num_heads, head_dim] -> [num_segments, max_seqlen, num_heads, head_dim]
            q_batch = q.reshape(num_segments, max_seqlen, self.num_heads, head_dim).transpose(1, 2)
            k_batch = k.reshape(num_segments, max_seqlen, self.num_heads, head_dim).transpose(1, 2)
            v_batch = v.reshape(num_segments, max_seqlen, self.num_heads, head_dim).transpose(1, 2)
            # [num_segments, num_heads, max_seqlen, head_dim]
            attn_output = F.scaled_dot_product_attention(q_batch, k_batch, v_batch, dropout_p=0.0)
            # [num_segments, num_heads, max_seqlen, head_dim] -> [seq_length, num_heads, head_dim]
            attn_output = attn_output.transpose(1, 2).reshape(seq_length, self.num_heads, head_dim)
        else:
            # General path: pad segments to max_seqlen, batch attend, then unpad
            # Pre-allocate padded batch tensors
            q_batch = q.new_zeros(num_segments, max_seqlen, self.num_heads, head_dim)
            k_batch = k.new_zeros(num_segments, max_seqlen, self.num_heads, head_dim)
            v_batch = v.new_zeros(num_segments, max_seqlen, self.num_heads, head_dim)
            # Build padding mask: True = valid token, False = padding
            pad_mask = torch.zeros(num_segments, max_seqlen, device=q.device, dtype=torch.bool)
            ...
            for i in range(num_segments):
                ...
                k_batch[i, :slen] = k[start:start + slen]
                v_batch[i, :slen] = v[start:start + slen]
                pad_mask[i, :slen] = True

            # Transpose to [num_segments, num_heads, max_seqlen, head_dim]
            q_batch = q_batch.transpose(1, 2)
            k_batch = k_batch.transpose(1, 2)
            v_batch = v_batch.transpose(1, 2)

            # Expand mask: [num_segments, 1, 1, max_seqlen] for key masking
            attn_mask = pad_mask[:, None, None, :]
            attn_mask = attn_mask.expand(-1, self.num_heads, max_seqlen, -1)

            attn_output = F.scaled_dot_product_attention(q_batch, k_batch, v_batch, attn_mask, dropout_p=0.0)
            # [num_segments, num_heads, max_seqlen, head_dim]
            attn_output = attn_output.transpose(1, 2)  # [num_segments, max_seqlen, num_heads, head_dim]

            # Unpad: gather valid tokens back to flat sequence
            parts = []
            for i in range(num_segments):
                parts.append(attn_output[i, :seg_lens[i]])
            attn_output = torch.cat(parts, dim=0)  # [seq_length, num_heads, head_dim]
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        skip_lm_head: bool = False,  # Return hidden states without lm_head (saves memory)
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_latent is not None:
                ...
                if n_image_tokens != n_image_features:
                    ...
                    # Split features per image using image_grid_thw_latent (each row = one image)
                    # After PatchMerger, feature count per image = t * (h // merge) * (w // merge)
                    num_images = image_grid_thw_latent.shape[0]
                    spatial_merge_size = self.visual.spatial_merge_size
                    features_per_image = (
                        image_grid_thw_latent[:, 0]
                        * (image_grid_thw_latent[:, 1] // spatial_merge_size)
                        * (image_grid_thw_latent[:, 2] // spatial_merge_size)
                    ).tolist()

                    assert n_image_tokens == num_images * latent_size, (
                        f"n_image_tokens ({n_image_tokens}) != num_images ({num_images}) * latent_size ({latent_size})"
                    )

                    if self.model.config.compress_strategy == "average":
                        # Compress each image's features independently to latent_size tokens
                        compressed_parts = []
                        offset = 0
                        for img_idx in range(num_images):
                            n_feat = features_per_image[img_idx]
                            img_feats = latent_image_embeds[offset:offset + n_feat]
                            offset += n_feat

                            # Trim to be divisible by latent_size
                            res = img_feats.shape[0] % latent_size
                            if res > 0:
                                img_feats = img_feats[:-res]

                            group_size = img_feats.shape[0] // latent_size
                            img_feats = img_feats.view(latent_size, group_size, hidden_states)
                            img_feats = img_feats.mean(dim=1)  # [latent_size, hidden]
                            compressed_parts.append(img_feats)

                        latent_image_embeds = torch.cat(compressed_parts, dim=0).contiguous()

                    else:
                        # Truncate each image's features to latent_size tokens
                        truncated_parts = []
                        offset = 0
                        for img_idx in range(num_images):
                            n_feat = features_per_image[img_idx]
                            img_feats = latent_image_embeds[offset:offset + min(n_feat, latent_size)]
                            offset += n_feat
                            truncated_parts.append(img_feats[:latent_size])

                        latent_image_embeds = torch.cat(truncated_parts, dim=0).contiguous()
        ...
        if skip_lm_head:
            # Return hidden states directly — caller will apply lm_head selectively
            return Qwen2_5_VLCausalLMOutputWithPast(
                loss=None,
                logits=None,
                past_key_values=outputs.past_key_values,
