...
## global value (lazy loaded on first use)
_dataset = None
_key2idx_load = None

def _load_hpdv3_globals():
    global _dataset, _key2idx_load
    if _dataset is None:
        _dataset = load_from_disk(os.environ.get("HPDV3_IMAGES_PATH", "./data/hpdv3/images"))
    if _key2idx_load is None:
        with open(os.environ.get("HPDV3_KEY2IDX_PATH", "./data/hpdv3/images_key2idx.json")) as f:
            _key2idx_load = json.load(f)

# Backwards-compat aliases (populated lazily)
dataset = None
key2idx_load = None

def resize_image_keep_ratio(image, max_size=640):
    """
    Resize image: if any side exceeds max_size, scale the long side to max_size proportionally.
    
    Args:
        image: PIL.Image object
        max_size: Maximum size limit, default 640
    ...
    # If both dimensions are within max_size, return original
    ...
    # Perform resize using LANCZOS algorithm for quality
...
class VLMGRPOTrainer(Trainer):
    ...
    def _get_key_from_inputs_adapted(self, x, key):
        """Adapted method that directly returns PIL Image objects."""
        _load_hpdv3_globals()
        image_paths = self._get_key_from_inputs(x, key)  # Get original image_path list
        images = []

        for image_path in image_paths:
            idx1 = _key2idx_load[image_path]
            pil_image = _dataset[idx1]['jpg']  # Should already be PIL Image object
            # Resize image
            resized_image = resize_image_keep_ratio(pil_image, max_size=640)
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for x in inputs:
            ...
            if "image_path" in x and x["image_path"] is not None:
                ...
                imgs = self._get_key_from_inputs_adapted(x, "image_path")
                # imgs = [PIL.Image.open(p) for p in self._get_key_from_inputs_adapted(x, "image_path")]
            else:
                imgs = []
