...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if self.config.stage == 'stage2' and not generate_mode:
            ...
            # 对于 tie_word_embeddings=False 的模型（如 7B），lm_head 是独立参数，
            # bf16 下反向传播的矩阵乘法累加会溢出，导致梯度 NaN。
            # 用 F.linear + 禁用 autocast 确保在 float32 下计算，彻底避免溢出。
            if not self.config.tie_word_embeddings:
                with torch.autocast(device_type='cuda', enabled=False):
                    logits = torch.nn.functional.linear(
                        hidden_states.float(),
                        self.lm_head.weight.float(),
                        self.lm_head.bias.float() if self.lm_head.bias is not None else None
                    )
            else:
