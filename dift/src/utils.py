...
def get_args():
    ...
    parser.add_argument("--exp_name", type=str, required=True, help="Experiment name for organizing outputs")
    parser.add_argument("--model", type=str, default='Qwen/Qwen2.5-VL-7B-Instruct')
    parser.add_argument("--stage", type=str, default="stage1")
    parser.add_argument("--task", type=str, default="vsp-spatial-reasoning", choices=["vsp-spatial-reasoning", "vsp-spatial-planning", "blink-jigsaw", "sat", "4dthinker"])
    ...
    parser.add_argument("--fps", type=float, default=1.0, help="Video frame rate (frames per second) for temporal sampling")
    parser.add_argument("--deepspeed_config", type=str, default=None, help="Path to DeepSpeed config JSON for multi-GPU training with ZeRO/TP")
    parser.add_argument("--local_rank", type=int, default=-1, help="Local rank for distributed training (passed by DeepSpeed launcher)")

    args = parser.parse_args()

    args.save_model_path = os.path.join(args.output_dir, args.exp_name, "checkpoints")
    args.log_file = os.path.join(args.output_dir, args.exp_name, "log.txt")
    args.logging_dir = os.path.join(args.output_dir, args.exp_name, "tensorboard")
...
def place_output_image(text, image_pad="<|vision_start|><|image_pad|><|vision_end|>", latent_placeholder="<output_image>", sep_token="<|im_start|>assistant") -> str:
    ...
    if latent_placeholder in text:
        idx = text.find(sep_token)
        if idx != -1:
            before_assistant = text[:idx + len(sep_token)]
            after_assistant = text[idx + len(sep_token):]
            after_assistant = after_assistant.replace(image_pad, '')
            after_assistant = after_assistant.replace(latent_placeholder, image_pad)
            text = before_assistant + after_assistant
        else:
...
def remove_user_images(examples):
    """Remove user visual content, for building assistant view (assistant image/video only)."""
    new_examples = []
...
def remove_assistant_images(examples):
    """Remove assistant visual content, for building user view (user image/video only)."""
    new_examples = []
    for example in examples:
        new_example = []
        for turn in example:
